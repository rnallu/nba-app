...
class VideosList extends React.Component {
      ...
      <div className="video_item" key={i}>
        <Link to={`/videos/${item.id}`}>
          <div className="videoList_container">
            <div className="image_titleDiv">
              <img
                src={require(`../../../images/videos/${item.image}`)}
                alt=""
                height="90px"
                width="110px"
              />
              <img
                className="image_play"
  ...
  render() {
    return (
      <div>
        <h2 className="video_heading">
          <strong>NBA</strong> Videos
        </h2>
        {this.loadVideos()}
        <div className="load_videos" onClick={this.loadMore}>
          Load More Videos
        </div>
      </div>
    );
